...
export const getListCategories = async (PAGE?: number, LIMIT?: number) =>
    await axiosClient.get(`${urlCategory}?page=${PAGE}&limit=${LIMIT}`);

export const getListCategoriesTrash = async (PAGE: number, LIMIT: number, softDelete: boolean) =>
    await axiosClient.get(`${urlCategory}?page=${PAGE}&limit=${LIMIT}&softDelete=${softDelete}`);
