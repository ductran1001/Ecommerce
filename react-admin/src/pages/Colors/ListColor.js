...
const ListColor = () => {
    ...
    const mutationDelete = (0, react_query_1.useMutation)({ mutationFn: (id) => Apis.destroyColor(id) });
    const query = (0, react_query_1.useQuery)({
        queryKey: [softDelete ? 'colors-trash' : 'colors', PAGE],
        queryFn: () => softDelete
            ? Apis.getListColorsTrash(PAGE, LIMIT, true)
            : Apis.getListColors(PAGE, LIMIT),
    ...
    return (<div className="flex flex-col h-full gap-8 px-4 pt-12">
            ...
            <div className="pb-8">
                <div className="flex gap-2">
                    <react_router_dom_1.Link to="/admin/colors/create" className="flex items-center w-12 h-12">
                ...
                <div className="max-w-2xl mx-auto my-8 overflow-x-auto">
                    <table className="w-full text-left text-gray-500 bg-white shadow-md sm:rounded-lg">
                        ...
                        <tbody>
                            {response &&
            ((_b = response.contents) === null || _b === void 0 ? void 0 : _b.map((color, index) => (<tr key={index} className="border-b hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 capitalize max-w-[180px] text-center">
                                            <div className="w-8 h-8 border" style={{ backgroundColor: color.code }}></div>
                                        </td>
                                        <td className="gap-2 px-6 py-4">
                                            <div className="flex items-center mb-2">
                                                <react_router_dom_1.Link to={`/admin/colors/update/${color._id}`} className="mr-2 font-medium text-blue-600 hover:underline">
                                                    <div>
                                                        <img src="/icon/edit-color.png" className="w-6 h-6" alt=""/>
                                                    </div>
                                                </react_router_dom_1.Link>
                                                <button onClick={() => handleDelete(color._id)} className="font-medium text-red-600 hover:underline">
                                                    <div>
