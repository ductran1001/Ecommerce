...
const UpdateColor = () => {
    ...
    const [color, setColor] = react_1.default.useState(Utils.initialStateColorForm);
    const { register, handleSubmit, formState: { errors, isValid }, } = (0, react_hook_form_1.useForm)({
        resolver: (0, yup_1.yupResolver)(Utils.createColorSchema),
        mode: 'onChange',
        defaultValues: color,
        values: color,
        resetOptions: {
    ...
    const mutationUpdate = (0, react_query_1.useMutation)({
        mutationFn: (body) => Apis.updateColor(id, body),
    });
    ...
    const queryColor = (0, react_query_1.useQuery)({
        queryKey: ['color', id],
        queryFn: () => Apis.getColorById(id),
        enabled: id !== undefined,
        onSuccess: (data) => {
            setColor((prevState) => (Object.assign(Object.assign({}, prevState), data.data.contents)));
        },
    });
    if (queryColor.error)
        return Layouts.Error(queryColor.error.message);
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="lg:w-3/5 w-full border py-8 px-4 sm:px-6 md:px-8 lg:px-10 mx-auto space-y-6">
                            ...
                            <div>
                                ...
                                <div className="flex items-end justify-end w-full">
                                    {(errors === null || errors === void 0 ? void 0 : errors.code) && (<p className="mt-2 text-sm italic md:w-3/4 w-full text-red-600 capitalize">
                                            {errors.code.message}
                                        </p>)}
                                </div>
                            </div>
