...
const categories_api_1 = require("api/categories.api");
...
const UpdateCategory = () => {
    ...
    const mutationUpdate = (0, react_query_1.useMutation)({
        mutationFn: (body) => Apis.updateCategory(id, body),
    });
    ...
    const queryCategory = (0, react_query_1.useQuery)({
        queryKey: ['category', id],
        queryFn: () => (0, categories_api_1.getCategoryById)(id),
        enabled: id !== undefined,
        onSuccess: (data) => {
            setCategory((prevState) => (Object.assign(Object.assign({}, prevState), data.data.contents)));
        },
    });
    if (queryCategory.error)
        return Layouts.Error(queryCategory.error.message);
    return (<div className="flex flex-col h-full gap-10 px-4 pt-12">
            {queryCategory.isLoading || mutationUpdate.isLoading ? <Layouts.Loading /> : null}
            <Layouts.Breadcrumb page="categories" sub="update"/>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="w-full px-4 py-8 mx-auto space-y-6 border lg:w-3/5 sm:px-6 md:px-8 lg:px-10">
                            ...
                            <div className="flex items-center">
                                <label className="w-1/4 text-sm text-gray-600">Active</label>
                                <div className="flex flex-1 gap-4 text-gray-600">
